...
public class ExcelTool<T> {
    ...
    public void exportExcel(List<Column> listTpamscolumn, List<T> datas,
                            ServletOutputStream fOut, boolean flag, boolean rowFlag) throws Exception {
        splitDataToSheets(datas, listTpamscolumn, flag, rowFlag);
        save(this.workbook, fOut);
    }
     ...
     * @throws Exception
     */
    private void writeSheet(Sheet sheet, List<T> data, List<Column> listTpamscolumn, boolean flag, boolean rowFlag) throws Exception {
    ...
    private void splitDataToSheets(List<T> data, List<Column> listTpamscolumn, boolean flag, boolean rowFlag) throws Exception {
        ...
        for (int i = 1; i <= pieces; i++) {
            Sheet sheet = this.workbook.createSheet(this.title + i);
     ...
     */
    private void writeSheetContent(List<Column> listTpamscolumn, List<T> datas, Sheet sheet, int rowIndex, boolean rowFlag) throws Exception {
        Row row = null;
     ...
     * @param t
     */
    private void createColl(Row row, int j, String finame, T t) {
     ...
     * @throws Exception
     */
    public void createCol(Row row, Column tpamscolumn, T v) throws Exception {
        Cell cell = row.createCell(tpamscolumn.getCol());  //创建单元格
        cell.setCellStyle(this.styleBody); //设置单元格样式
...
//========上部分是导出excel的使用（生成excel），下部分是解析excel，由于excel导入==================================================================================================================================
     ...
     * @return
     */
    public Sheet createHead(Sheet sheetCo, int r, int c) {
        for (int i = 0; i < r; i++) {
            Row row = sheetCo.createRow(i);
            for (int j = 0; j < c; j++) {
    ...
    public void createHead(List<Column> listTpamscolumn, Sheet sheetCo, int rowIndex) {
        ...
        for (int i = 0; i < len; i++) {//i是headers的索引，n是Excel的索引 多级表头
            ...
            RegionUtil.setBorderRight(BorderStyle.THIN, cra, sheetCo); // 右边框
