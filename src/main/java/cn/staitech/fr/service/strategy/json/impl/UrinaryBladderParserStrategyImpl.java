...
public class UrinaryBladderParserStrategyImpl implements ParserStrategy {
    ...
    private static Annotation processJsonElement(JsonNode element, ExecutorService executorService, Map<String, Long> pathologicalMap, JsonTask jsonTask) {
        ...
        try {
            Future<Annotation> future = executorService.submit(() -> UrinaryBladderParserStrategyImpl.handleSingleJsonElement(element, pathologicalMap, jsonTask));
...
//                    }
    ...
    public void alculationIndicators(JsonTask jsonTask) {
        ...
        SingleSlide singleSlide = singleSlideMapper.selectById(jsonTask.getSingleId());//组织轮廓面积
        Annotation annotation = new Annotation();
        annotation.setSequenceNumber(0L);//todo
        annotation.setSingleSlideId(jsonTask.getSingleId());//单脏器切片id
        annotation.setCategoryId(jsonTask.getCategoryId());// 标注类别ID
        Annotation structure = annotationMapper.getStructureArea(annotation);//膀胱腔面积
        BigDecimal singleSlideArea = new BigDecimal(singleSlide.getArea());
        BigDecimal structureArea = new BigDecimal(structure.getArea());
        BigDecimal result = singleSlideArea.subtract(structureArea);
        indicatorResultsMap.put("膀胱面积", new IndicatorAddIn("Urinary bladder area", String.valueOf(result), "平方毫米"));
