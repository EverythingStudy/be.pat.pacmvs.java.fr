...
public class MammaryGlandParserStrategyImpl extends AbstractCustomParserStrategy {
    ...
    public void alculationIndicators(JsonTask jsonTask) {
        ...
        //H-组织轮廓
        BigDecimal h = commonJsonParser.getOrganArea(jsonTask, "123111").getStructureAreaNum();
        ...
        // G皮肤-皮肤面积
        BigDecimal organAreaG = commonJsonParser.getOrganArea(jsonTask, "121111").getStructureAreaNum();
        // 毛囊密度 个/mm2 4=D/G
        ...
        if (subtract.signum() == 0) {
            ...
            map.put("结缔组织面积占比", new IndicatorAddIn("Connective tissue area%", DecimalUtils.percentScale3(subtract1.divide(subtract, 7, RoundingMode.HALF_UP)), "%"));
        }
        ...
        map.put("腺泡或导管细胞核密度（单个）", new IndicatorAddIn("Nucleus density of acinus or ducts （per）", confidenceInterval, SQ_UM_PICE));
        BigDecimal divide2 = new BigDecimal(organAreaCount2).divide(organArea1, 3, RoundingMode.HALF_UP);
        map.put("乳腺细胞核密度（全片）", new IndicatorAddIn("Nucleus density of mammary gland（all）", divide2.toString(), SQ_MM_PIECE));
        map.put("乳腺面积", new IndicatorAddIn("Mammary gland area", DecimalUtils.setScale3(h.subtract(organAreaA).subtract(organAreaB)), SQ_MM));

        map.put("淋巴结面积", new IndicatorAddIn("Lymph node area", DecimalUtils.setScale3(organAreaA), SQ_MM, CommonConstant.NUMBER_1));
        // 皮肤面积	G	平方毫米	此数据使用乳腺中皮肤数据 (乳腺皮肤公用)
        map.put("皮肤面积", new IndicatorAddIn("Skin area", DecimalUtils.setScale3(organAreaB), SQ_MM, CommonConstant.NUMBER_1));
        map.put("乳腺腺泡/导管数量", new IndicatorAddIn("Number of acinus and ducts", organAreaCount.toString(), "个", CommonConstant.NUMBER_1));
        map.put("乳腺腺泡/导管面积（全片）", new IndicatorAddIn("Breast acinar/ductal area (all)", DecimalUtils.setScale3(organArea1), SQ_MM, CommonConstant.NUMBER_1));
        map.put("结缔组织面积", new IndicatorAddIn("Connective tissue area", DecimalUtils.setScale3(organArea2), SQ_MM, CommonConstant.NUMBER_1));
        map.put("组织轮廓面积", new IndicatorAddIn("Organizational contour area", DecimalUtils.setScale3(h), SQ_MM, CommonConstant.NUMBER_1));
        ...
        if (organAreaG.compareTo(BigDecimal.ZERO) != 0) {
            ...
            BigDecimal stratumCorneumAreaRate = organArea3.divide(organAreaG, 7, RoundingMode.HALF_UP);
            ...
            BigDecimal nucleatedCellLayerAreaRate = organArea4.divide(organAreaG, 7, RoundingMode.HALF_UP);
        ...
        map.put("皮肤面积", new IndicatorAddIn("Skin area", DecimalUtils.setScale3(organAreaB), SQ_MM));
        map.put("皮肤面积", new IndicatorAddIn("Skin area", DecimalUtils.setScale3(organAreaG), "平方毫米"));
