...
/**
 * 
* @ClassName: IleumParserStrategyImpl
* @Description-d:回肠
* @author wanglibei
* @date 2025年7月21日
* @version V1.0
...
public class IleumParserStrategyImpl implements ParserStrategy {
    ...
    public void alculationIndicators(JsonTask jsonTask) {
         ...
         */
        // 肠腔面积	A	平方毫米
        map.put("肠腔面积", new IndicatorAddIn("Intestinal cavity area", DecimalUtils.setScale3(colonArea),CommonConstant.SQUARE_MILLIMETRE, CommonConstant.NUMBER_1,"117156"));

        // 黏膜层面积	B	平方毫米	以C型或双层环状输出
        map.put("黏膜层面积", new IndicatorAddIn("Mucosal layer area", DecimalUtils.setScale3(areaNumB),CommonConstant.SQUARE_MILLIMETRE, CommonConstant.NUMBER_1,"117008"));

        // 黏膜下层面积	C	平方毫米	以C型或双层环状输出
        map.put("黏膜下层面积", new IndicatorAddIn("Submucosal area", DecimalUtils.setScale3(areaNumC),CommonConstant.SQUARE_MILLIMETRE, CommonConstant.NUMBER_1,"117009"));

        // 肌层面积	D	平方毫米	以C型或双层环状输出
        map.put("肌层面积", new IndicatorAddIn("Muscle layer area", DecimalUtils.setScale3(areaNumD),CommonConstant.SQUARE_MILLIMETRE, CommonConstant.NUMBER_1,"11700C"));
...
//        map.put("组织轮廓面积", new IndicatorAddIn("Tissue area", DecimalUtils.setScale3(tissueArea),CommonConstant.SQUARE_MILLIMETRE, CommonConstant.NUMBER_1,"117111"));
        ...
        if (subtractFA.compareTo(BigDecimal.ZERO) != 0) {
            ...
            map.put("黏膜层面积占比", new IndicatorAddIn("Mucosal area%", DecimalUtils.percentScale3(mucosalAreaRateDecimal), "%",areaUtils.getStructureIds("117008","117111","117156")));

            // 黏膜下层面积占比 2	%	Submucosal area%	2=C/（F-A）
            BigDecimal submucosalAreaRateDecimal = areaNumC.divide(subtractFA, 7, RoundingMode.HALF_UP);
            map.put("黏膜下层面积占比", new IndicatorAddIn("Submucosal area%", DecimalUtils.percentScale3(submucosalAreaRateDecimal), "%",areaUtils.getStructureIds("117009","117111","117156")));

            // 肌层面积占比 3	%	Muscular area%	3=D/（F-A）
            BigDecimal muscularAreaRateDecimal = areaNumD.divide(subtractFA, 7, RoundingMode.HALF_UP);
            map.put("肌层面积占比", new IndicatorAddIn("Muscular area%", DecimalUtils.percentScale3(muscularAreaRateDecimal), "%",areaUtils.getStructureIds("11700C","117111","117156")));
            ...
            map.put("淋巴小结面积占比", new IndicatorAddIn("Lymphatic nodule area%", DecimalUtils.percentScale3(lymphaticNoduleAreaRateDecimal), "%",areaUtils.getStructureIds("117049","117111","117156")));
        } else {
            map.put("黏膜层面积占比", new IndicatorAddIn("Mucosal area%", "0.000", "%",areaUtils.getStructureIds("117008","117111","117156")));
            map.put("黏膜下层面积占比", new IndicatorAddIn("Submucosal area%", "0.000", "%",areaUtils.getStructureIds("117009","117111","117156")));
            map.put("肌层面积占比", new IndicatorAddIn("Muscular area%", "0.000", "%",areaUtils.getStructureIds("11700C","117111","117156")));
            map.put("淋巴小结面积占比", new IndicatorAddIn("Lymphatic nodule area%", "0.000", "%",areaUtils.getStructureIds("117049","117111","117156")));
        }

        // 回肠面积	5	平方毫米	Ileum area	5=F-A
        map.put("回肠面积", new IndicatorAddIn("Ileum area", DecimalUtils.setScale3(subtractFA),CommonConstant.SQUARE_MILLIMETRE, CommonConstant.NUMBER_0,areaUtils.getStructureIds("117111","117156")));
