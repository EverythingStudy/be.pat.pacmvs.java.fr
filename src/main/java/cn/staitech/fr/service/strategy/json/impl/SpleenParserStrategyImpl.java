...
//        BigDecimal accurateAreaDecimal = new BigDecimal(accurateArea);
        BigDecimal accurateAreaDecimal = commonJsonParser.getOrganArea(jsonTask, "145111").getStructureAreaNum();
        ...
        // 算法输出指标 -------------------------------------------------------------
        // A
        map.put("白髓面积", new IndicatorAddIn("White pulp area", DecimalUtils.setScale3(whitePulpArea), SQ_MM, CommonConstant.NUMBER_1));
        // B
        map.put("动脉周围淋巴鞘面积", new IndicatorAddIn("Periarterial lymphatic sheath area", DecimalUtils.setScale3(periarterialLymphaticSheathArea), SQ_MM, CommonConstant.NUMBER_1));
        // C
        map.put("中央动脉面积", new IndicatorAddIn("Central artery area", DecimalUtils.setScale3(centralArteryArea), SQ_MM, CommonConstant.NUMBER_1));
        // D
        map.put("含铁血黄素面积", new IndicatorAddIn("Hemosiderin area", DecimalUtils.setScale3(hemosiderinArea), SQ_MM, CommonConstant.NUMBER_1));
        // E
        map.put("红细胞面积", new IndicatorAddIn("Marginal zone area", DecimalUtils.setScale3(erythrocyteArea), SQ_MM, CommonConstant.NUMBER_1));
        // G
        map.put("边缘区面积", new IndicatorAddIn("Marginal zone area", DecimalUtils.setScale3(marginalZoneArea), SQ_MM, CommonConstant.NUMBER_1));
        // H
        map.put("红髓面积", new IndicatorAddIn("Red pulp", DecimalUtils.setScale3(redPulpArea), SQ_MM, CommonConstant.NUMBER_1));
        ...
        if (accurateAreaDecimal.compareTo(BigDecimal.ZERO) != 0) {
            ...
            map.put("白髓面积占比", new IndicatorAddIn("White pulp area%", DecimalUtils.percentScale3(whitePulpAreaRate), PERCENTAGE));
            ...
            map.put("含铁血黄素面积占比", new IndicatorAddIn("Hemosiderin area%", DecimalUtils.percentScale3(hemosiderinAreaRate), PERCENTAGE));
            ...
            map.put("红髓面积占比", new IndicatorAddIn("Red pulp area%", DecimalUtils.percentScale3(redPulpAreaRate), PERCENTAGE));

            // 红细胞面积占比	4	%	Erythrocyte area%	4=E/F
            BigDecimal erythrocyteAreaRate = erythrocyteArea.divide(accurateAreaDecimal, 7, RoundingMode.HALF_UP);
            map.put("红细胞面积占比", new IndicatorAddIn("Erythrocyte area%", DecimalUtils.percentScale3(erythrocyteAreaRate), PERCENTAGE));
            ...
            map.put("边缘区面积占比", new IndicatorAddIn("Marginal zone area", DecimalUtils.percentScale3(marginalZoneAreaRate), PERCENTAGE));
            ...
            map.put("动脉周围淋巴鞘面积占比", new IndicatorAddIn("Periarterial lymphatic sheath area%", DecimalUtils.percentScale3(periarterialLymphaticSheathAreaRate), PERCENTAGE));

        } else {
            map.put("白髓面积占比", new IndicatorAddIn("White pulp area%", "0.000", PERCENTAGE));
            map.put("含铁血黄素面积占比", new IndicatorAddIn("Hemosiderin area%", "0.000", PERCENTAGE));
            map.put("红髓面积占比", new IndicatorAddIn("Red pulp area%", "0.000", PERCENTAGE));
            map.put("红细胞面积占比", new IndicatorAddIn("Erythrocyte area%", "0.000", PERCENTAGE));
            map.put("边缘区面积占比", new IndicatorAddIn("Marginal zone area", "0.000", PERCENTAGE));
            map.put("动脉周围淋巴鞘面积占比", new IndicatorAddIn("Periarterial lymphatic sheath area%", "0.000", PERCENTAGE));
        }

        // F
        map.put("脾脏面积", new IndicatorAddIn("Spleen area", DecimalUtils.setScale3(accurateAreaDecimal), SQ_MM));
