...
public class ParotidGland_3ParserStrategyImpl extends AbstractCustomParserStrategy {
    ...
    public void alculationIndicators(JsonTask jsonTask) {
        ...
        // 算法输出指标
        indicatorResultsMap.put("导管面积（单个）", new IndicatorAddIn("", DecimalUtils.setScale3(ductAreaPer), "10³平方微米", CommonConstant.NUMBER_1, "30906F"));
        ...
        if (slideArea.compareTo(BigDecimal.ZERO) != 0) {
            ...
            indicatorResultsMap.put("导管面积占比（全片）", new IndicatorAddIn("Ducts area% (all)", DecimalUtils.percentScale3(ductAreaRatioAll), "%", areaUtils.getStructureIds("30906F", "309111")));

            // 腺泡面积占比 = C / E
            BigDecimal acinusAreaRatio = commonJsonParser.getProportion(acinusArea, slideArea);
            indicatorResultsMap.put("腺泡面积占比", new IndicatorAddIn("Acinus area%", DecimalUtils.percentScale3(acinusAreaRatio), "%", areaUtils.getStructureIds("30906D", "309111")));
        }
        ...
        if (acinusArea.compareTo(BigDecimal.ZERO) != 0) {
            ...
            indicatorResultsMap.put("腺泡细胞核密度", new IndicatorAddIn("Nucleus density of acinar cell", DecimalUtils.setScale3(acinusNucleusDensity), "个/平方毫米", areaUtils.getStructureIds("30906E", "30906D")));
        }
