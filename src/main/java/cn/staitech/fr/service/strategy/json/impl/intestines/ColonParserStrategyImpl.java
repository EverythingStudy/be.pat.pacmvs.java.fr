...
public class ColonParserStrategyImpl implements ParserStrategy {
    ...
    public void alculationIndicators(JsonTask jsonTask) {
        ...
        if (eSubtractA.compareTo(BigDecimal.ZERO) != 0) {
            ...
            String mucosalAreaRate = areaNum.divide(eSubtractA, 3, RoundingMode.HALF_UP).multiply(new BigDecimal(100)).setScale(3, RoundingMode.HALF_UP).toString();
            map.put("黏膜层面积占比", new IndicatorAddIn("Mucosal area%", mucosalAreaRate, "%"));

            // 黏膜下层面积占比	2	%	Submucosal area%	2=C/（E-A）	无
            String submucosalAreaRate = areaNum2.divide(eSubtractA, 3, RoundingMode.HALF_UP).multiply(new BigDecimal(100)).setScale(3, RoundingMode.HALF_UP).toString();
            map.put("黏膜下层面积占比", new IndicatorAddIn("Submucosal area%", submucosalAreaRate, "%"));

            // 肌层面积占比	3	%	Muscular area%	3=D/（E-A）	无
            String muscularAreaRate = areaNum3.divide(eSubtractA, 3, RoundingMode.HALF_UP).multiply(new BigDecimal(100)).setScale(3, RoundingMode.HALF_UP).toString();
        ...
        map.put("结肠面积", new IndicatorAddIn("Colon area", eSubtractA.setScale(3, RoundingMode.HALF_UP).toString(), "平方毫米", CommonConstant.NUMBER_0));
